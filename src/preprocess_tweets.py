...
def preprocess_tweets(data, lowercase=True, normalize=True, emojis='demojize', user_handles='@USER', urls='original_urls'):
    ...
    if emojis:
        data["text"] = data["text"].apply(partial(replace_emojis, replace=emojis))

    if user_handles:
        data["text"] = data["text"].apply(partial(replace_user_handles, replace=user_handles))

    if urls == 'original_urls':
        data["text"] = data[["text", 'processed_urls']].apply(lambda row: replace_urls(*row), axis=1)

    elif urls:
        data["text"] = data["text"].apply(partial(replace_urls, replace=urls))
